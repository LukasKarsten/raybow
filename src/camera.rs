...
use crate::{ray::Ray, raybow::WorkerState, vector::Vector};
...
impl Camera {
    ...
    pub fn get_ray(&self, s: f32, t: f32, state: &mut WorkerState) -> Ray {
...
fn random_lens_position(state: &mut WorkerState) -> Vector {
    let [angle, len, ..] = state.gen_random_floats();
